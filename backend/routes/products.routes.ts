...
export async function productRoutes(fastify: FastifyInstance) {
	...
	fastify.get(
		...
		async (request, reply) => {
		try {
			const query = request.query as { categoryId?: string; isActive?: string; lowStock?: string; search?: string; page?: string; limit?: string }
			...
			return { success: true, data: products.data, pagination: products.pagination }
		} catch (err: unknown) {
	...
	fastify.post('/:id/image', { preHandler: authMiddleware }, async (request, reply) => {
		try {
			const { id } = request.params as { id: string }
	...
	fastify.get(
		...
		try {
			...
			const products = await productService.getTopSelling(limit ? Number(limit) : undefined)
			return { success: true, data: products }
		} catch (err: unknown) {
	...
	// Import CSV
	fastify.post('/import', { preHandler: authMiddleware }, async (request, reply) => {
		try {
