...
import { settingSchema } from '../utils/validators.js'
...
export async function settingRoutes(fastify: FastifyInstance) {
	...
	fastify.get('/:key', { preHandler: authMiddleware }, async (request, reply) => {
		try {
	...
	fastify.put('/:key', { preHandler: [authMiddleware, requireRole(['admin'])] }, async (request, reply) => {
		try {
			const { key } = request.params as { key: string }
	...
	fastify.post('/reset', { preHandler: [authMiddleware, requireRole(['admin'])] }, async (request, reply) => {
		try {
