...
import { customerSchema, updateCustomerSchema } from '../utils/validators.js'
...
export async function customerRoutes(fastify: FastifyInstance) {
	fastify.get(
		...
		async (request, reply) => {
		try {
			const query = request.query as { query?: string; isActive?: string; page?: string; limit?: string }
			...
			return { success: true, data: customers.data, pagination: customers.pagination }
		} catch (err: unknown) {
	...
	fastify.get('/search', { preHandler: authMiddleware }, async (request, reply) => {
		try {
			...
			if (!q) {
				return reply.status(400).send({ success: false, error: { code: 'VALIDATION_ERROR', message: 'Query parameter "q" is required' } })
			}
	...
	fastify.get('/:id/history', { preHandler: authMiddleware }, async (request, reply) => {
		try {
			const { id } = request.params as { id: string }
