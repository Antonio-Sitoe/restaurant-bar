...
import { userService, type LoginCredentials, type CreateUserInput } from '../services/user.service.js'
import { loginSchema, createUserSchema, updateUserSchema } from '../utils/validators.js'
import { authMiddleware, requireRole } from '../middleware/auth.middleware.js'
import { handleError } from '../utils/errors.js'
import { logger } from '../utils/logger.js'
import jwt from 'jsonwebtoken'
import { createSwaggerSchema, zodToJsonSchema } from '../utils/swagger.js'
...
export async function userRoutes(fastify: FastifyInstance) {
	...
	fastify.put(
		...
		try {
			...
			const user = await userService.update(Number(id), body as Partial<CreateUserInput>)
			return { success: true, data: user }
		} catch (err: unknown) {
