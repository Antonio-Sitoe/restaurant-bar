...
export async function systemRoutes(fastify: FastifyInstance) {
	fastify.post('/backup', { preHandler: [authMiddleware, requireRole(['admin'])] }, async (request, reply) => {
		try {
	...
	fastify.post('/restore', { preHandler: [authMiddleware, requireRole(['admin'])] }, async (request, reply) => {
		try {
	...
	fastify.get('/info', { preHandler: authMiddleware }, async (request, reply) => {
		try {
			return {
				success: true,
				data: {
	...
	fastify.get('/logs', { preHandler: [authMiddleware, requireRole(['admin'])] }, async (request, reply) => {
		try {
	...
	fastify.get('/backups', { preHandler: [authMiddleware, requireRole(['admin'])] }, async (request, reply) => {
		try {
	...
	fastify.post('/backup/schedule', { preHandler: [authMiddleware, requireRole(['admin'])] }, async (request, reply) => {
		try {
