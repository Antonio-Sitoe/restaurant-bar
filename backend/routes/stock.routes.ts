...
import { stockMovementSchema, stockAdjustSchema } from '../utils/validators'
...
export async function stockRoutes(fastify: FastifyInstance) {
  ...
  fastify.post(
    ...
    async (request, reply) => {
      try {
        ...
        const adjustment = await stockService.adjust(
          body.productId,
          body.newQuantity,
          body.reason,
          authRequest.user?.id
  ...
  fastify.get(
    ...
    async (request, reply) => {
      try {
        const { productId, limit, page } = request.query as {
          productId?: string
          limit?: string
          page?: string
        }
  ...
  fastify.post(
    ...
    async (request, reply) => {
      try {
        const { counts } = request.body as {
          counts: Array<{ productId: number; quantity: number }>
        }
